...
fn encode((decimal_number, base): (char, usize)) -> String {
    ...
    if base >= max_base {
        ...
        format!(
            "{}{}",
            CENTRE_NUMERALS[base],
            BASE10_NUMERALS[base].repeat((digit - 5) as usize)
        )
    } else if digit == 4 {
...
fn main() {
    ...
    let input = &args[1];
    for roman_numeral in input.chars().zip((0..input.len()).rev()).map(encode) {
        writer
            .write_all(roman_numeral.as_bytes())
